package com.prohitman.onefarmmod.client.menu;
...
public class BreederMenu extends AbstractContainerMenu {
    ...
    public BreederMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public BreederMenu(int pContainerId, Inventory inv, BlockEntity entity) {
        super(ModMenuTypes.BREEDER_MENU.get(), pContainerId);
        checkContainerSize(inv, 1);
        blockEntity = ((BreederBlockEntity) entity);
        this.level = inv.player.level();
        ((BreederBlockEntity) entity).startOpen(inv.player);

        addPlayerInventory(inv);
        addPlayerHotbar(inv);

        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new SlotItemHandler(iItemHandler, 0, 80, 26));
        });
    }
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, OneFarmMod.BREEDER_BLOCK.get());
    }
